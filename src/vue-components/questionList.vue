        ...
        <ul>
            <li v-for="question in listQuestions">
                <div class="row" :style="saveAttempted && !entryUtil.isAnswerValid(entry, question.id, listQuestions) ? 'border: 1px solid red' : ''">
                    ...
                    <select class="six columns answer-select" @change="chooseAnswerInternal(question, $event)" v-model="entry.answers[question.id].answerId" :id="'dropdown' + question.id.split(' ').join('')">
